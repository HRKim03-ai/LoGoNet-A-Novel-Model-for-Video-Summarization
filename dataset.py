...
Canonical Split (no cross-validation)
"""
...
class VideoSummarizationDataset(Dataset):
    ...
    Uses Canonical Split (fixed train/test split, no cross-validation)
    ...
    def __init__(self, mode='train', dataset_path='dataset/mr_hisum.h5', 
                 split_file='dataset/mr_hisum_split.json', split_id=0):
        """
        Args:
            mode: 'train', 'val', or 'test'
            dataset_path: path to mr_hisum.h5 file
            split_file: path to split JSON file
            split_id: which canonical split to use (default: 0, no cross-validation)
        """
        self.mode = mode
        ...
        # Get video keys for this mode (canonical split)
        # If split_data has nested structure with split_id, use it
        # Otherwise, use direct keys (single canonical split)
        if isinstance(self.split_data.get(f'{mode}_keys'), list):
            # Direct keys (single split)
            self.video_keys = self.split_data.get(f'{mode}_keys', [])
        elif isinstance(self.split_data.get(f'{mode}_keys'), dict):
            # Nested structure with multiple splits
            self.video_keys = self.split_data.get(f'{mode}_keys', {}).get(str(split_id), [])
        else:
